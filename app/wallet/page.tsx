...
export default function Page() {
  ...
  const createWalletETH = (mnemonic: string) => {
    try {
      setLoading(true);
  ...
  const createWalletSOL = (mnemonic: string) => {
    try {
      setLoading(true);
      const seed = mnemonicToSeedSync(mnemonic);

      let derivativePath = `m/44'/501'/${wallets.length}'/0'`;
  ...
  const deleteWallet = (indexToDelete: number) => {
    setWallets((prevWallets) => {
      const updatedWallet = prevWallets.filter(
  ...
  return (
    <div className="w-full md:h-fit h-fit flex justify-center py-5 grid_background_dark">
      <div className="md:w-4/6 w-5/6 md:h-fit h-fit bg-[#111111] flex flex-col justify-between items-center rounded-lg md:p-6 p-3 cursor-pointer">
        {wallets?.map((wallet, index) => (
          <div className="md:py-0 py-4 px-1 md:px-4 my-4 w-full" key={index}>
            <div className="w-full flex justify-between items-center">
              <p
                className="text-white font-semibold text-lg md:text-xl py-2"
                onClick={() => navigateToViewWallet(wallet.address)}
              >
                Wallet {index + 1}
              </p>
              <button
                className="bg-red-600 p-2 text-sm rounded-lg hover:bg-red-700 transition duration-200 ease-in-out text-white"
            ...
            <div className="w-full flex flex-col">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-white mx-1 p-1 hover:bg-[#575757] rounded-lg transition duration-200 ease-in-out "
                >
                  copy
                </button>
              </div>
        ...
        <button
          onClick={() =>
            wallets[0].publicKey === wallets[0].address
              ? createWalletSOL(mnemonic)
              : createWalletETH(mnemonic)
          }
          className="my-10 bg-blue-600 p-3 rounded-lg hover:bg-blue-700 transition duration-200 ease-in-out text-white flex justify-center items-center"
        >
          {loading ? <Loader /> : "Add New Wallet"}
        </button>
      </div>
    </div>
  );
